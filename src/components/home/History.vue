<template>
  <ButtonPrimary
    type="button"
    class="ml-2 font-medium"
    @click="showModal = true"
  >
    Historial
  </ButtonPrimary>

  <ModalContainer
    className="max-w-3xl !w-5/6"
    v-show="showModal"
    @close="closeModal"
  >
    <div class="flex flex-col gap-y-4">
      <h2 class="text-center font-semibold text-lg">Historial</h2>
      <template v-if="showSelectDayData">
        <button
          class="font-medium text-start"
          @click="showSelectDayData = false"
        >
          {{ formattedDay(selectedDay) }} > rutinas
        </button>
        <div class="flex flex-col gap-y-3">
          <div
            class="flex flex-col gap-y-2"
            v-for="routine in progress[selectedDay]"
          >
            <span class="text-end w-full">{{ routine.date }}</span>
            <div
              class="flex flex-col bg-custom-gray-1 py-2 px-4 rounded-lg"
              v-for="exercise in routine.exercises"
              :key="exercise.id"
            >
              <span class="font-medium">{{ exercise.name }}</span>
              <div
                class="flex"
                v-for="(serie, serieIdx) in exercise.series"
                :key="serie.id"
                v-show="serie.weight > 0 && serie.reps > 0"
              >
                Serie {{ serieIdx + 1 }}: {{ serie.weight }}
                {{ serie.weightUnit }} x {{ serie.reps }}
              </div>
              <span
                class="text-sm text-pretty text-center"
                v-show="routine.exercises.length > 0"
              >
                No se ha encontrado información
              </span>
            </div>
          </div>

          <span
            class="text-center text-sm text-pretty"
            v-show="!progress[selectedDay].length"
          >
            Parece que no haz realizado ninguna rutina este dia.
          </span>
        </div>
      </template>
      <div class="flex flex-col gap-y-2" v-else>
        <ButtonSecondary
          class="w-full text-start"
          v-for="(_, day) in 7"
          :key="day"
          @click="selectDay(day)"
        >
          {{ formattedDay(day) }}
        </ButtonSecondary>
      </div>
    </div>
  </ModalContainer>
</template>

<script setup lang="ts">
import { ref } from "vue";
import ButtonPrimary from "../ui/buttons/ButtonPrimary.vue";
import ModalContainer from "../ui/generals/ModalContainer.vue";
import ButtonSecondary from "../ui/buttons/ButtonSecondary.vue";
import type { TDay } from "@/interfaces/progress";
import formattedDay from "@/utils/FormattedDay";
import { useProgressStore } from "@/stores/progress";
import { storeToRefs } from "pinia";

const showModal = ref(false);
const showSelectDayData = ref(false);
const selectedDay = ref<TDay>(0);

const progressStore = useProgressStore();
const { progress } = storeToRefs(progressStore);

const selectDay = (day: number) => {
  selectedDay.value = day as TDay;
  showSelectDayData.value = true;
};

const closeModal = () => {
  showSelectDayData.value = false;
  showModal.value = false;
};
</script>
